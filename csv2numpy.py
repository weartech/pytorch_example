    ...
    for element in elements:
        ...
        if(os.path.isdir(path)): # If element is a directory
            ...
            if(len(currInstances) == 0):
                currInstances = [temp]
                currLabels = element[0]
            else:
                print(len(currInstances))
                print(len([temp]))
                currInstances = np.vstack((currInstances, [temp]))
                currLabels = np.vstack((currLabels, element[0]))
...
if __name__ == "__main__":
    ...
    np.save(rootPath + "/data/instances", instances)
    np.save(rootPath + "/data/labels", labels)
